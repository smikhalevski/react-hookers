...
export function getNumberEncoding(format: Intl.NumberFormat): NumberEncoding {
  ...
  if (numberEncoding === undefined) {
    numberEncoding = {
      digitCodePointMap: getDigitCodePointMap(format),
      decimalCodePoints: getDecimalCodePoints(format),
      minusSignCodePoints: getMinusSignCodePoints(format),
    };
 ...
 */
function getDigitCodePointMap(format: Intl.NumberFormat): Map<number, string> {
 ...
 */
function getDecimalCodePoints(format: Intl.NumberFormat): Set<number> {
  const formatOptions = format.resolvedOptions();
 ...
 */
function getMinusSignCodePoints(format: Intl.NumberFormat): Set<number> {
  const formatOptions = format.resolvedOptions();
  const codePoints = new Set<number>();
...
export function encodeNumberValueParts(format: Intl.NumberFormat, str: string, sign: 1 | -1): NumberValueParts {
  ...
  const formatOptions = format.resolvedOptions();
  const decimalIndex = str.indexOf(DECIMAL_CHAR);

  if (formatOptions.maximumFractionDigits !== undefined && decimalIndex !== -1) {
    // Truncate fraction
    str = str.substring(0, decimalIndex + 1 + formatOptions.maximumFractionDigits);
  }
  ...
  const decimalFormat = new Intl.NumberFormat(formatOptions.locale, {
    ...formatOptions,
    maximumSignificantDigits: undefined,
    minimumFractionDigits: decimalIndex === -1 ? 0 : Math.max(1, str.length - decimalIndex - 1),
  ...
  if (decimalIndex === str.length - 1) {
    // Remove fraction parts but keep the decimal separator
